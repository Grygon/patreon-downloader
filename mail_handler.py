...
def get_emails(days_back_max, days_back_range):
    ...
    with MailBox('imap.gmail.com').login(EMAIL_UN, EMAIL_PW) as mailbox:
        mails = [m for m in mailbox.fetch(AND(from_="bingo@patreon.com",sent_date_lt=date_min,sent_date_gte=date_max))]
...
def get_post_urls(mail):
    try:
        ...
        URL = soup.find(string=re.compile(r"(View\sall\s\d*\simages\son\sPatreon|(View|Listen)\son\sPatreon|Vote\snow|Watch\snow)")).find_parent("a")['href']
        return [URL]
        ...
        url_list = [l[0] for l in all_url_list if re.search(string=l[0], pattern=specific_links)]
